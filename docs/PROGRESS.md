...
- [완료] **A. 층마다 배치 초기화** — `Party.reset_for_floor()`
  배치 전부 걷기 + DP 시작값 복귀 + 재배치 코스트 누적 초기화. 명일방주 스테이지 단위와 동일
  **문제였던 것**: 유저 실제 플레이 로그에서 15~20분 한 판 동안 배치는 6회 + 철수 1회가 전부였다.
  1층에서 6명 깔면 4개 층 동안 아무 조작이 없었다 — "전략이 없다" 는 체감의 직접 원인
- [완료] **B. 층마다 지형** — `floors.json` 의 `layouts` (레인당 "GGH" 3글자, . = 막힘)
  1층 9칸 기본 / 2층 b레인 열1 막힘 / 3층 고지 5칸 / 4층 고지 1칸 / 5층 열1 두 곳 막힘 / 6층 보스
  `RunState.tile_kind()` 가 층 지형을 들고, Party·DeployField 가 그걸 따른다
  `BattleLayout.COLUMN_KIND` 상수 제거 — 종류는 이제 층 데이터가 정한다
- [완료] **보스 돌파 = 즉시 실패** — 보스가 아이다에 닿으면 그 판 종료
  이전엔 보스를 안 잡고 지나보내도 "적 0마리" 가 되어 층이 클리어됐다 (실제 플레이에서 발생)
- 검증: 층 전환 시 배치 0명·DP 10·재배치누적 초기화, 6개 층 지형 9/8/9/9/7/9칸,
  막힌 타일 거부, 보스 돌파 시 아이다 hp 0
## 가신 개별 스킬 (명일방주 SP)

- [완료] `hero_skills.json` 을 SP 체계로 재작성 — 충전 방식 3종은 명일방주 그대로
  `auto` 초당1 / `attack` 공격 시 1 / `hit` 피격 시 1
  방벽(리엔, 피격충전, 저지+2·방어×2 8초) / 집중사격(공격충전, 공격력×2 6초)
  일제사격(자동충전, 즉발 광역) / **돌격 신호**(카린, 공격충전, 즉발 DP+8)
  돌격 신호는 명일방주 뱅가드 스킬이 DP 를 버는 것과 같은 역할이다
- [완료] **자동 발동만 둔다.** 가신 8명이 각자 버튼을 가지면 GDD 4.4 "버튼 3개 / 한 손 조작" 이 무너진다
  컷인도 아이다 전용으로 남긴다 — 8명이 12초마다 컷인을 띄우면 화면이 컷인으로 덮인다
  가신 스킬은 유닛 위 이름 팝업 + 파티클로 표시
- [완료] **unit.gd 분해** (602줄 → 453줄, CLAUDE.md 500줄 금지)
  `stat_mods.gd` 88줄 — 버프/속성 컨테이너 (트리를 안 쓰는 순수 계산)
  `unit_skill.gd` 96줄 — SP 충전과 발동
  `unit_visual.gd` 85줄 — 스프라이트 교체·체력바 (표시 전용)
  검증: 6명 배치, 70초에 스킬 6회 자동 발동, 스프라이트·체력바 정상
...
- **큰 블록을 인덱스 슬라이싱으로 옮기지 말 것.** 섹션 순서가 바뀐 걸 모르고
  `s[:start] + new + s[end:]` 를 했다가 end < start 라 구간이 통째로 복제됐다 (602→752줄).
  함수 단위 문자열 치환을 쓰거나, 옮기기 전에 헤더 위치를 먼저 확인한다
- **리팩터 전에 동작하는 상태를 먼저 커밋한다.** 되돌릴 지점이 없으면 손으로 복구해야 한다
- **배속 측정 시 `time_scale` 만 올리면 투사체가 적을 통과한다.**
  프레임당 이동거리가 적 폭보다 커지기 때문. `Engine.physics_ticks_per_second` 를 같이 올려
  스텝당 거리를 유지해야 한다 (time_scale 12 면 physics 240)
